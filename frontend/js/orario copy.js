...
function setSection(ndx, cl) {
    sections[ndx] = cl;
    insertOrUpdateSections();
};

function setClassInSection(cl) {
    
    sections[ndx] = cl;
    insertOrUpdateSections();
};

function insertOrUpdateSections() {
    var school = JSON.parse(localStorage.getItem('school'));
    var nms = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'L', 'M', 'N', 'O', 'P', 'Q', 'R'];
    var sec = {};
    sec.schoolId = school.id;
    sec.groups = [];
    for(i=0; i<16; i++){
        for(j=0; j<5; j++){
            if(sections[i][j] == 1){
...
function formatTeachersTable(teachers) {
    ...
    var tableContent = '<thead>';
      tableContent += '<tr>';
      tableContent += '<th width=\"50%\">Nome docente</th>';
      tableContent += '<th width=\"50%\">Materia</th>';
      tableContent += '</tr>';
      tableContent += '</thead>';
      tableContent += '<tbody>';
      $.each(teachers, function (index, teacher) {
            tableContent += '<tr class="goto_view_teacher" data-id="' + teacher.id + '">';
            tableContent += '<td class=\"nm\" width=\"50%\">' + teacher.name + '</td>';
            tableContent += '<td class=\"subj\" width=\"50%\">' + teacher.subject + '</td>';
            tableContent += '</tr>';
      });
      tableContent += '</tbody>';
...
function formatSectionTable(studentGroups) {
    studentGroups.groups.forEach(element => {
        setClassinSection(element);
    });
};

function loadGroups(callbackFunction) {
    var school = JSON.parse(localStorage.getItem('school'));
    var theUrl =  BASE_URL + 'groups/school/' + school.id; 
    $.ajax({
        url: theUrl,
        type: 'GET',
        complete: function(response, status, xhr){
