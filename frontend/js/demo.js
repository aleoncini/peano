...
function solve(id) {
    ...
    $.ajax({
        ...
        complete: function(response, status, xhr){
            var ref = "orario.html?id=" + demouuid;
            window.location.href = ref;
        },
        error: function(response, status, xhr){
          $('#saveResult').html('<p class="text-danger"><strong>UH Oh! Something went wrong, round not saved</strong></p>');
          setTimeout(function() { $('#saveResult').html(''); }, 3000);
        }
    });
}
